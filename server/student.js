...
router.post('/registration', (req, res) => {
    const { first_name, last_name, email, password, phone_number, gdpr, tags, description, github, portfolio, linkedin, work_place, app_starts, app_ends } = req.body;
    ...
    const query = `
    INSERT INTO Student (first_name, last_name, email, password, phone_number, gdpr, description, github, portfolio, linkedin, work_place, app_starts, app_ends) 
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`; 

    
    //1. create the student; enter the hashed password into the db
    db.run(query, [first_name, last_name, email, hashed_password, phone_number, description, github, portfolio, linkedin, work_place, app_starts], app_ends, function(err) {
        if(err){
